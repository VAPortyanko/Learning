package by.pva.hibernate.part01.persistenceContext.filteringEntitiesAndAssociations.staticFiltering;
...
//		WHERE ( d.active = true and d.account_type = 'DEBIT' ) AND d.client_id = 1
		...
		client2.getCreditAccounts()
		       .stream()
		       .forEach(System.out::println);
		System.out.println("Debit accounts:");
		client2.getDebitAccounts()
		       .stream()
	           .forEach(System.out::println);
...
class Account {
	...
	@Override
	public String toString() {
		return "Account [id=" + id + ", client=" + client + ", type=" + type + ", amount=" + amount + ", rate=" + rate + ", active=" + active + "]";
	}
