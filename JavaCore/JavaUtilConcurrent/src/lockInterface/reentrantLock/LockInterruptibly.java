	...
	public void run() {
		...
		for(int i=0; i<10; i++){
			try {
				...
				if (lock.isHeldByCurrentThread()) // Здесь мы закрываем лок, так как он мог быть получен в методе lockIntrerruptibly, а затем вылетело исключение на методе Sleep.
					lock.unlock();
				return;
			}
		}
